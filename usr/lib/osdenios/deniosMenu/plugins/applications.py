...
class pluginclass(object):
    ...
    def __init__(self, deniosMenuWin, toggleButton, de):
        ...
        self.builder.add_from_file("/usr/share/osdenios/deniosmenu/applications.ui")
        ...
        try:
            # GSettings stuff
            self.settings = Gio.Settings("com.osdenios.deniosmenu.plugins.applications")
    ...
    def refresh_apt_cache(self):
        if self.useAPT:
            path = "%s/.osdenios/deniosMenu" % home
            if not os.path.exists(path):
                os.makedirs(path)
            subprocess.Popen(["/usr/lib/osdenios/deniosMenu/plugins/get_apt_cache.py", "%s/apt.cache" % path])
    ...
    def add_search_suggestions(self, text):
        ...
        if self.enableInternetSearch:
            self.add_suggestion("/usr/lib/osdenios/deniosMenu/search_engines/ddg.svg",
                                _("Search DuckDuckGo for %s") % text, None, self.search_ddg)
            self.add_suggestion("/usr/lib/osdenios/deniosMenu/search_engines/wikipedia.svg",
    ...
    def add_apt_filter_results(self, keyword):
        try:
            ...
            path = os.path.join(home, ".osdenios/deniosMenu/apt.cache")
    ...
    def searchPopup(self, widget, event):
        ...
        if self.enableInternetSearch:
            add_menu_item('/usr/lib/osdenios/deniosMenu/search_engines/ddg.svg', _("Search DuckDuckGo"), self.search_ddg)
            add_menu_item('/usr/lib/osdenios/deniosMenu/search_engines/wikipedia.svg', _("Search Wikipedia"), self.search_wikipedia)
        ...
        add_menu_item()
        add_menu_item('/usr/lib/osdenios/deniosMenu/search_engines/software.png', _("Find Software"), self.search_mint_software)
        add_menu_item('/usr/lib/osdenios/deniosMenu/search_engines/tutorials.png', _("Find Tutorials"), self.search_mint_tutorials)
        add_menu_item('/usr/lib/osdenios/deniosMenu/search_engines/hardware.png', _("Find Hardware"), self.search_mint_hardware)
        add_menu_item('/usr/lib/osdenios/deniosMenu/search_engines/ideas.png', _("Find Ideas"), self.search_mint_ideas)
        add_menu_item('/usr/lib/osdenios/deniosMenu/search_engines/users.png', _("Find Users"), self.search_mint_users)
    ...
    def search_mint_tutorials(self, widget):
        ...
        subprocess.Popen(["xdg-open", "https://community.osdenios.com/index.php/tutorial/search/0/%s" % text])
        self.deniosMenuWin.hide()

    def search_mint_ideas(self, widget):
        text = urllib.parse.quote(self.searchEntry.get_text().strip())
        subprocess.Popen(["xdg-open", "https://community.osdenios.com/index.php/idea/search/0/%s" % text])
        self.deniosMenuWin.hide()

    def search_mint_users(self, widget):
        text = urllib.parse.quote(self.searchEntry.get_text().strip())
        subprocess.Popen(["xdg-open", "https://community.osdenios.com/index.php/user/search/0/%s" % text])
        self.deniosMenuWin.hide()

    def search_mint_hardware(self, widget):
        text = urllib.parse.quote(self.searchEntry.get_text().strip())
        subprocess.Popen(["xdg-open", "https://community.osdenios.com/index.php/hardware/search/0/%s" % text])
        self.deniosMenuWin.hide()

    def search_mint_software(self, widget):
        text = urllib.parse.quote(self.searchEntry.get_text())
        subprocess.Popen(["xdg-open", "https://community.osdenios.com/index.php/software/search/0/%s" % text])
        self.deniosMenuWin.hide()
    ...
    def migrate_favorites(self):
        ...
        default_path = os.path.join("/usr/lib/osdenios/deniosMenu/applications.list")
        path = os.path.join(home, ".osdenios/deniosMenu/applications.list")
