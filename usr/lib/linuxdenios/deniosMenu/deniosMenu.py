...
# Rename the process
setproctitle.setproctitle('deniosmenu')
...
class MainWindow(object):
    ...
    def __init__(self, toggleButton, settings, de):
        ...
        builder = Gtk.Builder()
        builder.set_translation_domain("deniosmenu")
        builder.add_from_file("/usr/share/linuxdenios/deniosmenu/main.ui")
        ...
        plugindir = os.path.join(os.path.expanduser("~"), ".linuxdenios/deniosMenu/plugins")
            ...
            if plugin != "newpane":
                try:
                    ...
                    if X.pluginclass.__init__.__code__.co_argcount == 1:
                        ...
                        # pass deniosMenu and togglebutton instance so that the plugin can use it
...
class MenuWin(object):
    ...
    def __init__(self, applet, iid):
        ...
        self.settings = Gio.Settings.new("com.linuxdenios.deniosmenu")
        ...
        self.mainwin.window.set_name("deniosmenu") # Name used in Gtk RC files
        self.applyTheme()
    ...
    def updatePanelButton(self):
        ...
        if os.path.isfile("/etc/linuxdenios/info"):
            with open("/etc/linuxdenios/info") as info:
                for line in info:
        ...
        self.sizeButton()
        self.button_box.get_style_context().add_class('deniosmenu')
    ...
    def showAboutDialog(self, action, userdata = None):
        ...
        about.set_program_name("deniosMenu")
        about.set_version("__DEB_VERSION__")
        about.set_license_type(Gtk.License.GPL_3_0)
        about.set_comments(_("Advanced MATE Menu"))
        # about.set_authors(["Clement Lefebvre <clem@linuxdenios.com>", "Lars-Peter Clausen <lars@laprican.de>"])
        about.set_translator_credits(("translator-credits"))
        # about.set_copyright(_("Based on USP from S.Chanderbally"))
        about.set_logo(GdkPixbuf.Pixbuf.new_from_file("/usr/lib/linuxdenios/deniosMenu/icon.svg"))
        about.connect("response", lambda dialog, r: dialog.destroy())
        about.show()

    def showPreferences(self, action, userdata = None):
        Execute("/usr/lib/linuxdenios/deniosMenu/preferences.py")
    ...
    def create_menu(self):
        ...
        action = Gtk.Action(name="DeniosMenuPrefs", label=_("Preferences"), tooltip=None)
        action.connect("activate", self.showPreferences)
        action_group.add_action(action)
        if os.path.exists("/usr/bin/menulibre") or os.path.exists("/usr/bin/mozo"):
            action = Gtk.Action(name="DeniosMenuEdit", label=_("Edit menu"), tooltip=None)
            action.connect("activate", self.showMenuEditor)
            action_group.add_action(action)
            menu_file = "popup.xml"
        action = Gtk.Action(name="DeniosMenuReload", label=_("Reload plugins"), tooltip=None)
        action.connect("activate", self.mainwin.RegenPlugins)
        action_group.add_action(action)
        action = Gtk.Action(name="DeniosMenuAbout", label=_("About"), tooltip=None)
        action.connect("activate", self.showAboutDialog)
        action_group.add_action(action)
        action_group.set_translation_domain ("deniosmenu")
...
MatePanelApplet.Applet.factory_main("DeniosMenuAppletFactory", True,
