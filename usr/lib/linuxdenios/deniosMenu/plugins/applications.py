...
class pluginclass(object):
    ...
    #@print_timing
    def __init__(self, deniosMenuWin, toggleButton, de):
        self.deniosMenuWin = deniosMenuWin
        RecentHelper.deniosMenuWin = deniosMenuWin
        self.mainMenus = []
        ...
        self.builder.add_from_file("/usr/share/linuxdenios/deniosmenu/applications.ui")
        ...
        self.keyPress_handler = self.deniosMenuWin.window.connect("key-press-event", self.keyPress)
        ...
        try:
            # GSettings stuff
            self.settings = Gio.Settings("com.linuxdenios.deniosmenu.plugins.applications")
    ...
    def refresh_apt_cache(self):
        if self.useAPT:
            path = "%s/.linuxdenios/deniosMenu" % home
            if not os.path.exists(path):
                os.makedirs(path)
            subprocess.Popen(["/usr/lib/linuxdenios/deniosMenu/plugins/get_apt_cache.py", "%s/apt.cache" % path])
    ...
    def get_panel(self):
        ...
        for applet in applet_list:
            ...
            if "applet-iid" in keys:
                ...
                if iid is not None and iid.find("DeniosMenu") != -1:
    ...
    def destroy(self):
        ...
        self.deniosMenuWin.window.disconnect(self.keyPress_handler)
    ...
    def add_search_suggestions(self, text):
        ...
        if self.enableInternetSearch:
            self.add_suggestion("/usr/lib/linuxdenios/deniosMenu/search_engines/ddg.svg",
                                _("Search DuckDuckGo for %s") % text, None, self.search_ddg)
            self.add_suggestion("/usr/lib/linuxdenios/deniosMenu/search_engines/wikipedia.svg",
    ...
    def add_apt_filter_results(self, keyword):
        try:
            ...
            path = os.path.join(home, ".linuxdenios/deniosMenu/apt.cache")
    ...
    def searchPopup(self, widget, event):
        ...
        if self.enableInternetSearch:
            add_menu_item('/usr/lib/linuxdenios/deniosMenu/search_engines/ddg.svg', _("Search DuckDuckGo"), self.search_ddg)
            add_menu_item('/usr/lib/linuxdenios/deniosMenu/search_engines/wikipedia.svg', _("Search Wikipedia"), self.search_wikipedia)
        ...
        add_menu_item()
        add_menu_item('/usr/lib/linuxdenios/deniosMenu/search_engines/software.png', _("Find Software"), self.search_denios_software)
        add_menu_item('/usr/lib/linuxdenios/deniosMenu/search_engines/tutorials.png', _("Find Tutorials"), self.search_denios_tutorials)
        add_menu_item('/usr/lib/linuxdenios/deniosMenu/search_engines/hardware.png', _("Find Hardware"), self.search_denios_hardware)
        add_menu_item('/usr/lib/linuxdenios/deniosMenu/search_engines/ideas.png', _("Find Ideas"), self.search_denios_ideas)
        add_menu_item('/usr/lib/linuxdenios/deniosMenu/search_engines/users.png', _("Find Users"), self.search_denios_users)
    ...
    def search_denios_tutorials(self, widget):
        text = urllib.parse.quote(self.searchEntry.get_text().strip())
        subprocess.Popen(["xdg-open", "https://community.linuxdenios.com/index.php/tutorial/search/0/%s" % text])
        self.deniosMenuWin.hide()

    def search_denios_ideas(self, widget):
        text = urllib.parse.quote(self.searchEntry.get_text().strip())
        subprocess.Popen(["xdg-open", "https://community.linuxdenios.com/index.php/idea/search/0/%s" % text])
        self.deniosMenuWin.hide()

    def search_denios_users(self, widget):
        text = urllib.parse.quote(self.searchEntry.get_text().strip())
        subprocess.Popen(["xdg-open", "https://community.linuxdenios.com/index.php/user/search/0/%s" % text])
        self.deniosMenuWin.hide()

    def search_denios_hardware(self, widget):
        text = urllib.parse.quote(self.searchEntry.get_text().strip())
        subprocess.Popen(["xdg-open", "https://community.linuxdenios.com/index.php/hardware/search/0/%s" % text])
        self.deniosMenuWin.hide()

    def search_denios_software(self, widget):
        text = urllib.parse.quote(self.searchEntry.get_text())
        subprocess.Popen(["xdg-open", "https://community.linuxdenios.com/index.php/software/search/0/%s" % text])
        self.deniosMenuWin.hide()
    ...
    def favoritesBuildLauncher(self, location):
        try:
            ...
            if favButton.appExec:
                ...
                self.deniosMenuWin.setTooltip(favButton, favButton.getTooltip())
    ...
    def migrate_favorites(self):
        ...
        default_path = os.path.join("/usr/lib/linuxdenios/deniosMenu/applications.list")
        path = os.path.join(home, ".linuxdenios/deniosMenu/applications.list")

        if os.path.isdir(path):
            # dir created by a bug in denios 19.2 beta
            ...
            if addedApplications:
                ...
                for item in addedApplications:
                    ...
                    if item["button"].appExec:
                        self.deniosMenuWin.setTooltip(item["button"], item["button"].getTooltip())
